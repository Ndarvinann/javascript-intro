...
function updateStock() {
  ...
  if(newStock !==""&&newStock>0){//validation check. newStock !== " ensures the box isnt empty.  newStock > 0: Ensures the entered value is a positive number.

      kglstore[produce] = parseInt(newStock); // 
      document.getElementById(produce +"Stock").textContent = newStock; //.textContent = newStock: Updates the text content of the <span> element with the new stock value.
